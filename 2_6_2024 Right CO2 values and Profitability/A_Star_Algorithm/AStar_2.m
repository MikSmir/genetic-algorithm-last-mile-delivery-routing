function [Optimal_path, path_cost] = AStar_2(Startx, Starty, Targetx, Targety, targetnum, blockLength, blockWidth, streetWidth, numCustomers)
...
dispTarget = ['Target ',num2str(targetnum)];
dispDepot = ['Depot'];
...
for i=1:MAX_X
    for j=1:MAX_Y
        if(MAP(i,j) == 1)
            ...
            xTarget = i;
            yTarget = j;
                if(targetnum < numCustomers-1)
                    text(i+1,j+1,dispTarget)
                else
                    text(i+1,j+1,dispDepot)
                end
        elseif(MAP(i,j) == -1)
           plot(i+.5,j+.5, 'r.', 'MarkerSize', 6);
        elseif(MAP(i,j) == 0)
            
            % plot(i+0.5,j+0.5,'bo'); % Plots a shifted duplicate target
            xval = i;
            xStart = i;
            yval = j;
            yStart = j;
